...
## Carregando os dados
Antes de começar, utilize a função load_faunabr para carregar os dados. Para informações mais detalhadas sobre como obter e carregar os dados, consulte [Getting started with faunabr](https://wevertonbio.github.io/faunabr/articles/primeiros_passos.html)
...
O filtro retornou 109 espécies que atendem aos critérios especificados. É importante notar que essas seleções incluem espécies com ocorrências confirmadas no Rio de Janeiro, e algumas delas também podem ter ocorrências confirmadas em outros estados.
...
Agora, o filtro retornou 20 espécies, todas exclusivamente confinadas ao Rio de Janeiro.
...
#> [1] "PR;RS;SC"
```
Agora, o filtro retornou apenas 132 espécies, todas ocorrendo em PR, SC e RS; sem registros de ocorrências em qualquer outro estado.

## Checagem de nomes científicos e subseleção de espécies
### Extraíndo o nome binomial
...
#> [5] "Arrenurus (Incertae sedis) tumulosus" "Araucaria angustifolia" 
```
...
#> 1 Zonotrichia capensis Correct  Zonotrichia c…        0 accepted_name   Zonotrichia… #> Passe…
#> 2 Mazama bororo        Correct  Mazama bororo         0 synonym         Mazama jucu… #> Cervi…
#> 3 Mazama jucunda       Correct  Mazama jucunda        0 accepted_name   Mazama jucu… #> Cervi…
#> 4 Arrenurus (Incertae… Correct  Arrenurus (In…        0 accepted_name   Arrenurus (… #> Arren…
#> 5 Pantera onça         Probabl… Panthera onca         2 accepted_name   Panthera on… #> Felid…
#> 6 Araucaria angustifo… Not_fou… NA                   NA NA              NA           NA 

```
Podemos ver que *Zonotrichia capensis*, *Mazama jucunda* e *Arrenurus (Incertae sedis) tumulosus* estão escritos corretamente e são nomes aceitos. *Mazama bororo* está escrito corretamente, mas é um sinônimo de *Mazama jucunda*.
...
accepted_names <- na.omit(accepted_names) #Remove NAs

#Subselecionar especies
my_sp <- subset_fauna(data = bf, species = accepted_names,
                      include_subspecies = FALSE)
tibble::tibble(my_sp)
#> # A tibble: 4 × 19
...
```{r}
my_sp2 <- subset_fauna(data = bf, species = accepted_names,
...
#> 49343     Panthera onca                  <NA>       accepted_name
