...
The filter returned 109 species that meet the specified criteria. It's important to note that these selections include species with confirmed occurrences in Rio de Janeiro, and some of them may also have confirmed occurrences in other states:
...
Now, the filter has resulted in 20 species, all exclusively confined to Rio de Janeiro.
...
#> [1] "PR;RS;SC"
```
Now, the filter return only 132 species, all of them occurring in PR, SC and RS; with no recorded occurrences in any other states.
...
```
We can see that *Zonotrichia capensis*, *Mazama jucunda*, and *Arrenurus (Incertae sedis)* are spelling correctly and are accepted names. *Mazama bororo*  is spelling correctly, but it is a synonym of *Mazama jucunda*.
...
The spelling of *Araucaria angustifolia* was flagged as Not_found (as expected, given that *Araucaria angustifo* is not an animal!). Consequently, the name was not located in the database, and there were no comparable names available.
